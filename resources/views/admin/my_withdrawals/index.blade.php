            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                <div class="item-card flex flex-row justify-between items-center">
                    ...
                    <div>
                        ...
                        <h3 class="text-indigo-950 text-xl font-bold">Rp {{ number_format($w->amount_requested, 0, ',', '.') }}</h3>
                    </div>
                    <div>
                        <p class="text-slate-500 text-sm">Date</p>
                        <h3 class="text-indigo-950 text-xl font-bold">{{ \Carbon\Carbon::parse($w->created_at)->format('d M Y') }}
                        </h3>
                    </div>
                    @if($w->has_set)
                    @if($w->has_received)
                    ...
                    <div class="hidden md:flex flex-row items-center gap-x-3">
                        <a href="{{route('admin.my_withdrawal.details',$w)}}" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                            View Details
                        </a>
                    </div>
                </div>
