            ...
            <div id="content" class="w-full bg-white rounded-t-[40px] flex flex-col gap-5 p-[30px_24px_120px]">
                <div class="flex flex-col gap-[10px]">
                   ...
                   @else
                    ...
                    <p  class="badge bg-[#76AE43] rounded-full p-[6px_12px] font-bold text-xs text-white
                     w-fit leading-[25px]">FINISHED</p>
                     @endif
                ...
                <div class="flex flex-col gap-[2px]">
                    ...
                    $shortText = Str::limit($fundraising->about, 20, '...'); // 100 karakter pertama
                @endphp
                
                <p class="desc-less text-sm leading-[26px]">
                    {{ $shortText }}
                    <button class="text-[#FF7815] underline" onclick="toggleSeeMoreLess()">see more</button>
                </p>
                <p class="desc-more text-sm leading-[26px] hidden">
                    {{ $fundraising->about }}
                ...
                <div class="flex flex-col gap-3">
                    <div class="flex items-center justify-between">
                        <h2 class="font-semibold text-sm">Supporters({{ $fundraising->donaturs->where('is_paid', true)->count() }})</h2>
                        <a href="" class="p-[6px_12px] rounded-full bg-[#E8E9EE] font-semibold text-sm">View All</a>
                    </div>
                    <div class="flex flex-col gap-4">
                        @forelse($fundraising->donaturs as $fd)
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex flex-col gap-[2px] w-full">
                                <div class="flex items-center justify-between">
                                    <p class="font-bold">Rp {{ number_format($fd->total_amount, 0, ',', '.') }}</p>
                                    <p class="font-semibold text-[10px] leading-[15px] text-right text-[#66697A]">by {{$fd->name}}</p>
                                </div>
                                <p class="caption text-xs leading-[18px] text-[#66697A]">“{{$fd->notes}}”</p>
                            </div>
                        </div>
             @empty
