...
@section('title','Most Famousn Fundraising Indonesia')
@section('content')
...
<section class="max-w-[640px] w-full min-h-screen mx-auto flex flex-col bg-white overflow-x-hidden pb-[134px]">
    ...
    <div id="popular-fundrising" class="mt-8">
        ...
        <div class="main-carousel mt-[14px]">
            ...
            <div class="px-2 first-of-type:pl-4 last-of-type:pr-4">
                <a href="{{route('front.category',$a)}}" class="fundrising-card rounded-[30px] w-[135px] min-h-[160px] flex flex-col items-center gap-3 p-5 border border-[#E8E9EE]">
                    <div class="w-[60px] h-[60px] flex shrink-0 overflow-hidden">
                        <img src="{{Storage::url($a->icon)}}" alt="icon">
                    </div>
                    <span class="font-semibold text-center my-auto">{{$a->name}}</span>
                </a>
            </div>
            @endforeach
        </div>
        ...
        <div class="main-carousel mt-[14px]">
            ...
            <div class="px-2 first-of-type:pl-4 last-of-type:pr-4 mb-6">
                <div class="flex flex-col gap-[14px] rounded-2xl border border-[#E8E9EE] p-[14px] w-[208px]">
                    <a href="{{route('front.details',$f)}}">
                        <div class="rounded-2xl w-full h-[120px] flex shrink-0 overflow-hidden">
                            <img src="{{Storage::url($f->thumbnail)}}" class="w-full h-full object-cover" alt="thumbnail">
                        </div>
                    </a>
                    <div class="flex flex-col gap-[6px]">
                        <a href="{{route('front.details',$f)}}" class="font-bold line-clamp-2 hover:line-clamp-none">{{$f->name}}</a>
                        <p class="text-xs leading-[18px]">Target <span class="font-bold text-[#FF7815]">Rp {{ number_format($f->target_amount, 0, ',', '.') }}</span></p>
                    </div>
                    <progress id="fund" value="{{$f->getPercentageAttribute()}}" max="100" class="w-full h-[6px] rounded-full overflow-hidden"></progress>
                </div>
            </div>
            @endforeach
        </div>
    ...
    <div id="latest-fundrising" class="mt-8">
        ...
        <div class="flex flex-col gap-4 mt-[14px] px-4">
            ...
            @foreach($fundraisings as $fun)
            <a href="{{route('front.details',$fun)}}" class="card">
                <div class="w-full border border-[#E8E9EE] flex items-center p-[14px] gap-3 rounded-2xl bg-white">
                    <div class="w-20 h-[90px] flex shrink-0 rounded-2xl overflow-hidden">
                        <img src="{{Storage::url($fun->thumbnail)}}" class="w-full h-full object-cover" alt="thumbnail">
                    </div>
                    <div class="flex flex-col gap-1">
                        <p class="font-bold line-clamp-1 hover:line-clamp-none">{{$fun->name}}</p>
                        <p class="text-xs leading-[18px]">Target <span class="font-bold text-[#FF7815]">Rp {{ number_format($fun->target_amount, 0, ',', '.') }}</span></p>
                        <div class="flex items-center gap-1 sm:flex-row-reverse sm:justify-end">
                            <p class="font-semibold sm:font-medium text-xs leading-[18px]">{{$fun->fundraiser->user->name}}</p>
